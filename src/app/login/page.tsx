...
export default function FanLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "SYNCED",
        description: "Welcome back to the void. Identity verified.",
      });
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "Sync Failed",
        description: error.message || "Invalid frequency parameters.",
      });
      setLoading(false);
    }
  };
      ...
      <div className="flex-grow flex items-center justify-center p-6 pt-32">
        <Card className="w-full max-w-md bg-[#1E201E]/80 border-white/10 rounded-none shadow-2xl relative z-10 backdrop-blur-2xl">
          ...
          <CardHeader className="text-center pt-16 pb-10">
            ...
            <p className="text-[10px] uppercase tracking-[0.6em] text-white/40 mt-6">RE-IDENTIFY YOUR PRESENCE</p>
          </CardHeader>

          <CardContent className="p-10 md:p-14">
            <form onSubmit={handleLogin} className="space-y-10">
              <div className="space-y-4 group">
                <label className="text-[10px] uppercase tracking-[0.4em] font-black text-white/40 group-focus-within:text-muted transition-colors">VOID_ID_EMAIL</label>
              ...
              <div className="space-y-4 group">
                <label className="text-[10px] uppercase tracking-[0.4em] font-black text-white/40 group-focus-within:text-muted transition-colors">SECURITY_KEY_PASSWORD</label>
                <div className="relative">
              ...
              >
                {loading ? (
                  ...
                  <span className="relative z-10 flex items-center gap-4">
                    ENTER_VOID <ArrowRight className="h-8 w-8 group-hover:translate-x-3 transition-transform" />
                  </span>
                )}
