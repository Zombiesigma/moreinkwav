...
export function OrderManager() {
  ...
  const deleteOrder = (orderId: string) => {
    if (confirm('Permanently remove this order history?')) {
      const orderRef = doc(db, 'orders', orderId);
      deleteDocumentNonBlocking(orderRef);
      toast({
        title: "ORDER_REMOVED",
        description: "Entry deleted from the void.",
      });
    }
  };
  ...
  const OrderCard = ({ order }: { order: any }) => {
    ...
    return (
      <Card className="bg-[#1E201E] border-white/5 rounded-none overflow-hidden hover:border-muted/30 transition-all duration-500 shadow-2xl group relative">
          ...
          <div className="lg:col-span-3 p-8 bg-black/40 border-r border-white/5 flex flex-col justify-between">
            ...
            {order.status !== 'completed' && (
              <div className="pt-8 grid grid-cols-2 gap-2">
                ...
                <Button size="sm" variant="ghost" className="h-10 px-4 text-[10px] text-white/20 hover:text-white" onClick={() => updateStatus(order.id, 'completed')}>FORCE DONE</Button>
              </div>
            )}
          </div>
    ...
    <div className="space-y-12">
      <div className="flex items-end justify-between border-b border-white/10 pb-6">
        <div>
          ...
          <p className="text-[10px] uppercase tracking-[0.4em] text-white/20 mt-2">Managing merch flow</p>
        </div>
      </div>

      <Tabs defaultValue="active" className="space-y-8">
        <TabsList className="bg-white/5 border border-white/10 p-1 h-auto rounded-none">
          <TabsTrigger value="active" className="data-[state=active]:bg-muted data-[state=active]:text-black rounded-none px-8 py-3 font-headline uppercase tracking-widest flex items-center gap-2">
            <Activity size={14} /> ACTIVE ({activeOrders.length})
          </TabsTrigger>
          <TabsTrigger value="archive" className="data-[state=active]:bg-muted data-[state=active]:text-black rounded-none px-8 py-3 font-headline uppercase tracking-widest flex items-center gap-2">
            <History size={14} /> ARCHIVE ({completedOrders.length})
          </TabsTrigger>
        </TabsList>

        <TabsContent value="active" className="space-y-10 animate-fade-in">
          {activeOrders.length > 0 ? (
            ...
            <div className="py-32 border-2 border-dashed border-white/5 text-center flex flex-col items-center justify-center opacity-10">
              <Package size={60} className="mb-6 text-muted" />
              <p className="font-headline text-3xl uppercase tracking-widest">NO_ACTIVE_FLOWS</p>
            </div>
          )}
        </TabsContent>

        <TabsContent value="archive" className="space-y-10 animate-fade-in">
          {completedOrders.length > 0 ? (
            completedOrders.map(order => <OrderCard key={order.id} order={order} />)
          ) : (
            <div className="py-32 border-2 border-dashed border-white/5 text-center flex flex-col items-center justify-center opacity-10">
              <History size={60} className="mb-6 text-muted" />
              <p className="font-headline text-3xl uppercase tracking-widest">ARCHIVE_EMPTY</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
